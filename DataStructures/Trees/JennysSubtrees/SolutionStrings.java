class Result2 {
    ...
    static boolean isSubtree(Node current, int radius) {
        ...
        String result = "";
        String canonical = canonize(subtree);
    ...
    static String canonize(Set<Integer> subtree) {
        ...
        if(subtree.size() == 2) {
            ...
            result = subtree.stream().map(g::get).findAny().orElseThrow().canonize().toString();
        }
...
/*        void concat(StringBuilder other) {
            if(other == null) return;
            if(canonicals == null)
                canonicals = other;
            if(other.compareTo(canonicals) < 0) {
                canonicals = other.append(canonicals);
            } else {
                canonicals.append(other);
            }
        } */
        final int name;
